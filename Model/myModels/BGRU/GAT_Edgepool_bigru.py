...
from torch.nn import GRU, AvgPool2d

class bigru_detect(nn.Module):
    def __init__(self, num_layers, hidden, nheads, nclass, dropout, alpha, training):
        super(bigru_detect, self).__init__()
        self.dropout = dropout
        self.num_classes = nclass
        self.training = training
        self.nheads = nheads

        self.bgru = GRU(hidden, 128, 2, batch_first=True, bidirectional=True)
    ...
    def forward(self, h1):
        bgru_data = torch.stack([h1], dim=1)
        out, h0 = self.bgru(bgru_data)
        out = self.pool(out)
        res = self.out(out[:, -1, :])
