...
import random
from sklearn.metrics import recall_score, precision_score, f1_score, roc_auc_score, roc_curve, confusion_matrix, accuracy_score, mean_absolute_error, mean_squared_error, r2_score
from myModels.bilstm.vul_detection import VulnerabilityDetection
from myModels.bilstm.GAT_Edgepool_bi_lstm import bi_lstm_detect
...
def graph_emb(data,epoch):
    ...
    data = desc_features,mess_features
    # h = model(data)
    return data
...
def test(testlist, model_index, ramData, batch_size):
    ...
    for i in Test_data_batches:
        ...
        for codepair in batches[i]:
            ...
            h1,h2 = graphEmbDict[test_data[0]]
            h1 = torch.as_tensor(h1).to(device)
            h2 = torch.as_tensor(h2).to(device)
            data = h1,h2
            ...
            output = bi_lstm_detection(data, model_index)
...
def train():
    ...
    iterations = 0
    acc_loss = open('acc_loss_lstm.csv', mode='a', encoding="utf=8", newline='')
    fieldnames = ["Accuracy", "loss"]
    writer = csv.DictWriter(acc_loss, fieldnames=fieldnames)
    writer.writeheader()
    test_rec = open('test_rec_lstm.csv', mode='a', encoding="utf=8", newline='')
    ...
    for epoch in epochs:
        ...
        print("save epoch")
        torch.save(model.state_dict(), './saveModels/epoch'+str(epoch+addNum)+'.pkl')
