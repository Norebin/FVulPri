...
from myModels.BGRU.bgru_vul_detection import VulnerabilityDetection
from myModels.BGRU.GAT_Edgepool_bigru import bigru_detect
...
def bi_lstm_detection(data,epoch):
    model = bigru_detect(args.num_layers, args.hidden, args.nheads, args.num_classes, args.dropout, args.alpha, False).to(device)
    saveModel = torch.load('./saveModels/gru/epoch'+str(epoch)+'.pkl')
    model_dict = model.state_dict()
    state_dict = {k:v for k,v in saveModel.items() if k in model_dict.keys()}
    #print(state_dict.keys())
    model_dict.update(state_dict)
...
def train():
    ...
    iterations = 0
    acc_loss = open('acc_loss_gru.csv', mode='a', encoding="utf=8", newline='')
    fieldnames = ["Accuracy", "loss"]
    writer = csv.DictWriter(acc_loss, fieldnames=fieldnames)
    writer.writeheader()
    test_rec = open('test_rec_gru.csv', mode='a', encoding="utf=8", newline='')
    ...
    for epoch in epochs:
        ...
        print("save epoch")
        torch.save(model.state_dict(), './saveModels/gru/epoch'+str(epoch+addNum)+'.pkl')
